...
class SegNet:
    def __init__(self, conf_file="config.json"):
        ...
        if self.use_vgg is False:
            ...
            print("No VGG path in config, so perform the learning from scratch")
        else:
        ...
        # HH: overrides the current default graph 
        self.graph = tf.Graph()
        with self.graph.as_default():
            ...
            # HH: edited
            # Before entering the images into the architecture, we need to do Local Contrast Normalization
            ...
            # ---------------------Let's start the decoder process---------------------------------------------------#
    ...
    def train(self, max_steps=30001, batch_size=3):
        ...
        # HH:edited
        # FOR TARAINING THE BAYES, FLAG_OPT SHOULD BE SGD, BUT FOR TRAINING THE NORMAL SEGNET,
        ...
        with self.graph.as_default():
            ...
            loss, accuracy, prediction = cal_loss(self.logits, self.labels_pl, self.num_classes)
            ...
            with self.sess.as_default():
                ...
                for step in range(max_steps):
                    ...
                    # HH added
                    if step<10:
                        print("Epoch {}: Train Loss{:6.3f}, Train Accu {:6.3f}".format(step, self.train_loss[-1],
                                                                                       self.train_accuracy[-1]))
                    # HH added
                    if step<100 and step>= 10 and step % 10 == 0:
                        print("Epoch {}: Train Loss{:6.3f}, Train Accu {:6.3f}".format(step, self.train_loss[-1],
                                                                                       self.train_accuracy[-1]))
                    # HH added
                    ...
                    if step % 1000 == 0:
                        ...
                        print(
                            "Epoch {}: Train Loss {:6.3f}, Train Acc {:6.3f}, Val Loss {:6.3f}, Val Acc {:6.3f}".format(
    ...
    def visual_results(self, dataset_type = "TEST", images_index = 3, FLAG_MAX_VOTE = False):
        ...
        with self.sess as sess:
            ...
            _, _, prediction = cal_loss(self.logits, self.labels_pl, self.num_classes)
            ...
            if (dataset_type=='TRAIN'):
                ...
                if type(images_index) == list:
                    ...
                    indexes = random.sample(range(600),images_index)
                ...
                if type(images_index) == list:
                    ...
                    indexes = random.sample(range(200),images_index)
            ...
            for image_batch, label_batch in zip(images,labels):
                ...
                if FLAG_BAYES is False:
                    ...
                    if FLAG_MAX_VOTE is True:
                        ...
                        var_one = var_calculate(pred,prob_variance, image_w, image_h)
                        pred = np.reshape(pred,[image_h,image_w])
    ...
    def visual_results_external_image(self, images, FLAG_MAX_VOTE = False):
        ...
        with self.sess as sess:
            ...
            for i in range(len(images)):
                labels.append(np.array([[1 for x in range(image_w)] for y in range(image_h)]))
